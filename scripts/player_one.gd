...
@onready var player_sprite = $AnimationPlayer
...
func handle_input():
	...
	if movement_direction.length() > 0:
		...
		player_sprite.play("walk_%s" % determine_animation_suffix(movement_direction))
	else:
		player_sprite.play("idle_%s" % idle_direction)
